...
def identify_dc_events(price_series: np.ndarray, threshold: float) -> List[Tuple[str, int, float, int, int]]:
    dc_events = []
    event = 'upturn'  # Initialize as upturn event
    ph = pl = price_series[0]
    tdc0 = tdc1 = tos0 = tos1 = 0

    for t, p in enumerate(price_series):
        if event == 'upturn':
            if p <= ph * (1 - threshold):
                # Downturn event confirmed
                event = 'downturn'
                pl = p
                tdc1 = t
                tos0 = t + 1
                dc_events.append(('downturn', tdc0, ph, tdc1, tos0))
            elif p > ph:
                # Update highest price during upturn
                ph = p
                tdc0 = t
                tos1 = t - 1
        else:  # event == 'downturn'
            if p >= pl * (1 + threshold):
                # Upturn event confirmed
                event = 'upturn'
                ph = p
                tdc1 = t
                tos0 = t + 1
                dc_events.append(('upturn', tdc0, pl, tdc1, tos0))
            elif p < pl:
                # Update lowest price during downturn
                pl = p
                tdc0 = t
                tos1 = t - 1
...
def identify_os_events(dc_events: List[Tuple[str, int, float, int, int]], price_series_length: int) -> List[Tuple[int, int]]:
    os_events = []
    for i in range(len(dc_events) - 1):
        os_start = dc_events[i][3] + 1  # Start immediately after the DC event
        os_end = dc_events[i + 1][4]    # End at the next DC event
        if os_start < os_end:
            os_events.append((os_start, os_end))
    # Handle the last OS period
    if dc_events:
        os_start = dc_events[-1][3] + 1
...
def visualize_dc_os(price_series: np.ndarray, dc_events: List[Tuple[str, int, float, int, int]], os_events: List[Tuple[int, int]]):
    """
    Visualize the price series with DC events and OS periods.

    Args:
    price_series (np.ndarray): Array of price values.
    dc_events (List[Tuple[str, int, float, int, int]]): List of DC events.
    os_events (List[Tuple[int, int]]): List of OS periods.
    """
    plt.figure(figsize=(15, 7))
    plt.plot(price_series, label='Price')

    for event_type, index, price, tdc0, tos0 in dc_events:
